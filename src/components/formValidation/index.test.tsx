...
import FormValidation, { TESTIDS, ERR_MESGS } from './index'
import userEvent from '@testing-library/user-event'

describe('Form Validation', () => {

  test('initial state with no error element present', () => {
    render(<FormValidation />)

    // There is no error
    expect(() => screen.getByTestId(TESTIDS.ERROR_MESG)).toThrow()

    const submitButton = screen.getByTestId(TESTIDS.SUBMIT_BUTTON)
    expect(submitButton).toBeDisabled()
  })
  test('cause email error and then correct it', async () => {
    render(<FormValidation />)

    const inputElement = screen.getByTestId(TESTIDS.EMAIL_INPUT)

    // input an invalid email
    await userEvent.type(inputElement, 'foo@bar')
    const errMesgField = screen.getByTestId(TESTIDS.ERROR_MESG)
    expect(within(errMesgField).getByText(ERR_MESGS.BAD_EMAIL)).toBeInTheDocument()

    // put in a valid email
    await userEvent.clear(inputElement)
    await userEvent.type(inputElement, 'foo@bar.com')
    expect(() => screen.getByTestId(TESTIDS.ERROR_MESG)).toThrow()
  })
  test('cause first name error and then correct it', async () => {
    render(<FormValidation />)

    const inputElement = screen.getByTestId(TESTIDS.FIRST_NAME_INPUT)

    // input an invalid email
    await userEvent.type(inputElement, 'Tom25')
    const errMesgField = screen.getByTestId(TESTIDS.ERROR_MESG)
    expect(within(errMesgField).getByText(ERR_MESGS.BAD_NAME)).toBeInTheDocument()

    // put in a valid email
    await userEvent.clear(inputElement)
    await userEvent.type(inputElement, 'Tom')
    // no error element
    expect(() => screen.getByTestId(TESTIDS.ERROR_MESG)).toThrow()
  })
  test('cause last name error and then correct it', async () => {
    render(<FormValidation />)

    const inputElement = screen.getByTestId(TESTIDS.LAST_NAME_INPUT)

    // input an invalid email
    await userEvent.type(inputElement, 'Landry24')
    const errMesgField = screen.getByTestId(TESTIDS.ERROR_MESG)
    expect(within(errMesgField).getByText(ERR_MESGS.BAD_NAME)).toBeInTheDocument()

    // put in a valid email
    await userEvent.clear(inputElement)
    await userEvent.type(inputElement, 'Landry')
    // no error element
    expect(() => screen.getByTestId(TESTIDS.ERROR_MESG)).toThrow()
  })
  test('All fields filled correctly enables submit button', async () => {
    render(<FormValidation />)

    let inputElement = screen.getByTestId(TESTIDS.FIRST_NAME_INPUT)
    await userEvent.type(inputElement, 'Tom')
    inputElement = screen.getByTestId(TESTIDS.LAST_NAME_INPUT)
    await userEvent.type(inputElement, 'Landry')
    inputElement = screen.getByTestId(TESTIDS.EMAIL_INPUT)
    await userEvent.type(inputElement, 'foo@bar.com')

    const submitButton = screen.getByTestId(TESTIDS.SUBMIT_BUTTON)
    expect(submitButton).toBeEnabled()
  })
  test('Submitted input outputs the values', async () => {
    render(<FormValidation />)
    ...
    const submitOutput = screen.getByTestId(TESTIDS.SUBMIT_OUTPUT)
    expect(within(submitOutput).getByText('Submitted Name: Tom Landry')).toBeInTheDocument()
    expect(within(submitOutput).getByText('Submitted Email: foo@bar.com')).toBeInTheDocument()
